defmodule Squads.SessionsTest do
  ...
  describe "list_sessions/0" do
    ...
    test "returns all sessions" do
      ...
      {:ok, _session1} =
        Sessions.create_session(%{agent_id: agent.id, ticket_key: "owner/repo#1"})

      {:ok, _session2} =
        Sessions.create_session(%{agent_id: agent.id, ticket_key: "owner/repo#2"})

      sessions = Sessions.list_sessions()
      assert length(sessions) == 2
      # Verify both ticket_keys are present
      ticket_keys = Enum.map(sessions, & &1.ticket_key)
      assert "owner/repo#1" in ticket_keys
      assert "owner/repo#2" in ticket_keys
    end
  end
  ...
  describe "list_sessions_for_agent/1" do
    test "returns sessions for specific agent only" do
      ...
      {:ok, _s1} = Sessions.create_session(%{agent_id: agent1.id, ticket_key: "owner/repo#1"})
      {:ok, _s2} = Sessions.create_session(%{agent_id: agent2.id, ticket_key: "owner/repo#2"})
  ...
  describe "list_sessions_by_status/1" do
    test "returns sessions filtered by status" do
      ...
      {:ok, s1} = Sessions.create_session(%{agent_id: agent.id, ticket_key: "owner/repo#1"})

      {:ok, _s2} =
        Sessions.create_session(%{
          agent_id: agent.id,
          ticket_key: "owner/repo#2",
          status: "running"
        })
  ...
  describe "create_session/1" do
    ...
    test "creates session with optional fields" do
      ...
      assert session.ticket_key == "owner/repo#42"
